...
namespace db
{
  ...
  //コンストラクタ
  TLyScVS::TLyScVS(TSnScenario* pScene)
    :TLyScGen(pScene)
    , ugvs_(this)
  {
  ...
  void TLyScVS::MvPage_VS()
  {
    if (field_->sc_IsCamEnter()) //さわったら即
    {
      ChangePage(pg_GotoShiai);
      pScene_->lyScBar_->MoveLayer(scl_Shiai);// SetFilter_GoShiai();//->stLy_.lyphase_ = 3;//shiaiへ
    }
  }
  void TLyScVS::MvPage_GotoShiai()
  {

  }

  void TLyScVS::DoUpdate(BOOL draw_f)
  {
    ugvs_.Update(draw_f);
